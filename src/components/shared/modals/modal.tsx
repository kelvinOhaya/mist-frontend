...
export function ModalButton({ text, highlight, onClick }: ModalButtonProps) {
  return (
    <button
      type="button"
      className={`flex-1 p-2 rounded-full ${highlight ? "bg-(--p300)" : "bg-transparent"} hover:cursor-pointer ${highlight ? "" : "border-2 border-(--neutral-accent)"}`}
      onClick={onClick}
    >
      {text}
    </button>
